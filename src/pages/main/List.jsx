...
const MusicList = () => {
    ...
    const handlePlaySelectedSongs = () => {
        ...
        // 팝업에 전달할 곡 데이터 준비
        const songParams = selectedSongs.map(song => ({
            title: encodeURIComponent(song.title),
            artist: encodeURIComponent(song.artistName),
            fileUrl: encodeURIComponent(song.fileUrl)
        }));

        // 첫 번째 곡으로 팝업을 열고 재생
        const firstSong = songParams[0];
        openPlayerPopup(firstSong.title, firstSong.artist, firstSong.fileUrl);
    };
